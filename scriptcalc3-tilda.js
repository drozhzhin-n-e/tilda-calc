...
(function (window, $) {
    ...
    function mountClipin(rootSelector) {
        ...
        function calculateCeilingClipIn() {
            ...
            var cassetteName = 'Панель потолочная BASMAN CN Clip-in, ' + cassetteSizeText.replace('×', 'х') + ', Zn, Th=0,5мм RAL 9003 мат порошков. покр.';
        ...
        if (window.TILDA_CALC_MULTI_MODE) {
            $root.find('.calc-cassette-type-option').each(function () {
                var oc = this.getAttribute('onclick');
                if (oc && /selectCassetteType\s*\(/i.test(oc)) {
            ...
            $root.off('click.tildaCalcCassette').on('click.tildaCalcCassette', '.calc-cassette-type-option', function () {
                selectCassetteType(this);
            });
            ...
            (function bindLegacyOnclick() {
                var pairs = [
                    { re: /^calculateCeilingClipIn\s*\(\s*\)$/i, fn: calculateCeilingClipIn },
